...
def train_nn_regression_model(learning_rate, steps, batch_size, hidden_units,
  ...
  """
    ...
    plt.legend()
    plt.savefig('Tensorflow/DNN/result.png')
    plt.close()
...
# YOUR CODE HERE
test_feature_samples = preprocess_features(california_housing_test_data)
test_target_samples = preprocess_targets(california_housing_test_data)

predict_test_input_fn = lambda: my_input_fn(
    test_feature_samples,
    test_target_samples["median_house_value"],
    num_epochs=1,
    shuffle=False)

predictions_test = dnn_regressor.predict(predict_test_input_fn)
predictions_test = np.array(
    [item['predictions'][0] for item in predictions_test])

# Compute training and validation loss.
test_root_mean_squared_error = math.sqrt(
    metrics.mean_squared_error(predictions_test, test_target_samples))
