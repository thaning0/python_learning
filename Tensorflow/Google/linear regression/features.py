...
if __name__ == "__main__":
    ...
    # Use new feasure created by latitude
    def select_and_transform_features(source_df):
        # Use zip tu create a iterator (32,33), (33,34), ..., (44,45)
        LATITUDE_RANGES = zip(range(32, 44), range(33, 45))
        selected_examples = pd.DataFrame()
        selected_examples["median_income"] = source_df["median_income"]
        selected_examples["rooms_per_person"] = source_df["rooms_per_person"]
        for r in LATITUDE_RANGES:
            selected_examples["latitude_%d_to_%d" %
                              r] = source_df["latitude"].apply(
                                  lambda val: 1 * (val >= r[0] and val < r[1]))
        return selected_examples

    selected_training_examples = select_and_transform_features(
        training_examples)
    selected_validation_examples = select_and_transform_features(
        validation_examples)
    ...
    california_housing_test_data = pd.read_csv(
        "https://download.mlcc.google.cn/mledu-datasets/california_housing_test.csv",
        sep=",")

    # compare predictions from test data with validation
    test_samples = preprocess_features(california_housing_test_data)
    test_targets = preprocess_targets(california_housing_test_data)

    selected_test_examples = select_and_transform_features(test_samples)
